...
# ===== add generalization directory to path =====
...
def build_base_config() -> BuildingConfig:
    """
    你可以把这里当作“固定基准建筑”，ML 的特征主要来自被扫描的参数。
    """
...
def default_sweeps():
    """
    你们之后做 RF/GBDT：这些就是候选特征的“采样空间”（可固定）。
    """
    return [
        # 外部几何（策略参数）
        dict(param_name="overhang_depth", shading_type="Overhang", facade_azimuth=0,
             values=[0.0, 0.3, 0.6, 0.9, 1.2, 1.5, 2.0, 2.5]),
        ...
        dict(param_name="window_ratio", shading_type="Overhang", facade_azimuth=0,
             values=[0.15, 0.25, 0.30, 0.35, 0.45, 0.55]),
        ...
        dict(param_name="ventilation_rate", shading_type="Overhang", facade_azimuth=0,
             values=[0.3, 0.5, 1.0, 2.0, 4.0]),

        # 热惯性（如果你们想让 ML 学到 thermal mass 的作用）
        dict(param_name="C_in", shading_type="Overhang", facade_azimuth=0,
             values=[200000.0, 500000.0, 800000.0, 1200000.0, 2000000.0]),
    ]
